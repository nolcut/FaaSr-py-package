...
def faasr_start(faasr_payload):
    ...
    # If the payload is a DAG, then this function returns an adjacency list for the workflow
    # If the payload is not a DAG, then the action aborts
    pre = check_dag(faasr_obj.get_payload_dict())

    # Get the predecessor functions for the current function invocation
    payload_dict = faasr_obj.get_payload_dict()
